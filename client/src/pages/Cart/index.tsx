  ...
  const handleOrder = async () => {
    ...
    const createdOrder = {
      ...
      items: cart.map((item) => ({
        ...
        ingredients: item.ingredients,
        quantity: item.quantity,
        comment: item.comment, // Behåll kommentarer från cart-item
        locked: item.locked, // Behåll locked-status från cart-item
        done: item.done, // Behåll done-status från cart-item
      })),
      totalPrice: calculateTotalPrice(),
      orderDate: new Date().toISOString(),
      locked: false,
